  ...
  return (
    <header className="px-4 lg:px-6 h-14 flex items-center !bg-white">
      <div className="container mx-auto max-w-7xl flex items-center justify-between">
        {!isHomePage && <Logo />}
        <nav className={`${isHomePage ? 'w-full justify-end' : 'ml-auto'} flex gap-4 sm:gap-6 items-center`}>
          {/* Desktop menu - only show if not on homepage */}
          {!isHomePage && (
          ...
          {/* Notifications - always show if user is logged in */}
          {user && (
            <NotificationsButton />
          )}
          
          {/* Mobile menu button - only show if not on homepage */}
          {!isHomePage && (
